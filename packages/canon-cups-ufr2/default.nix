# Ported from https://github.com/NixOS/nixpkgs/blob/nixos-23.11/pkgs/misc/cups/drivers/canon/default.nix
{
  dream2nix,
  config,
  lib,
...
let
  ...
  libs = lib.makeLibraryPath config.buildInputs;

  versionNoDots = builtins.replaceStrings [ "." ] [ "" ] version;

  src_canon = config.deps.fetchurl {
    url = "https://gdlp01.c-wss.com/gds/${dl}/linux-UFRII-drv-v${versionNoDots}-us-05.tar.gz";
    hash = "sha256-1abbvNSfuaY1HCubgE5eF4Pyq9YOHDTDCTcLgdFadeY=";
  };
in
{
  imports = [
  ...
  mkDerivation = {
    ...
    postUnpack = ''
      (
        cd $sourceRoot
        tar -xf Sources/cnrdrvcups-lb-${version}-1.05.tar.xz
